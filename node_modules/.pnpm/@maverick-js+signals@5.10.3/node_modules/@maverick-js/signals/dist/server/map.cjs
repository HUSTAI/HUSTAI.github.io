...
var chunkV5QCM7NN_cjs = require('./chunks/chunk-V5QCM7NN.cjs');

// src/selector.ts
function selector(source) {
  let currentKey, nodes = /* @__PURE__ */ new Map();
  chunkV5QCM7NN_cjs.effect(() => {
    const newKey = source(), prev = nodes.get(currentKey), next = nodes.get(newKey);
    prev && chunkV5QCM7NN_cjs.write.call(prev, false);
    next && chunkV5QCM7NN_cjs.write.call(next, true);
    currentKey = newKey;
  });
  return function observeSelector(key) {
    ...
    node.s += 1;
    chunkV5QCM7NN_cjs.onDispose(node);
    return chunkV5QCM7NN_cjs.read.bind(node);
  };
}
...
SelectorProto.r = chunkV5QCM7NN_cjs.isNotEqual;
...
function computedMap(list, map, options) {
  return chunkV5QCM7NN_cjs.read.bind(
    chunkV5QCM7NN_cjs.createComputation(
      [],
      updateMap.bind({
        o: chunkV5QCM7NN_cjs.createScope(),
        h: 0,
        t: list,
        g: [],
        p: map,
...
function updateMap() {
  let i = 0, newItems = this.t() || [], mapper = () => this.p(chunkV5QCM7NN_cjs.read.bind(this.c[i]), i);
  chunkV5QCM7NN_cjs.scoped(() => {
    if (newItems.length === 0) {
      if (this.h !== 0) {
        chunkV5QCM7NN_cjs.dispose.call(this.o, false);
        this.g = [];
        this.f = [];
        this.h = 0;
        this.c = [];
    ...
    for (i = 0; i < newItems.length; i++) {
      if (i < this.g.length && this.g[i] !== newItems[i]) {
        chunkV5QCM7NN_cjs.write.call(this.c[i], newItems[i]);
      } else if (i >= this.g.length) {
        this.f[i] = chunkV5QCM7NN_cjs.compute(
          this.c[i] = chunkV5QCM7NN_cjs.createComputation(newItems[i], null),
          mapper,
          null
        );
      }
    ...
    for (; i < this.g.length; i++)
      chunkV5QCM7NN_cjs.dispose.call(this.c[i]);
...
function updateKeyedMap() {
  ...
  chunkV5QCM7NN_cjs.scoped(() => {
    let newLen = newItems.length, i, j, mapper = indexed ? () => this.p(newItems[j], chunkV5QCM7NN_cjs.read.bind(this.c[j])) : () => this.p(newItems[j]);
    if (newLen === 0) {
      if (this.h !== 0) {
        chunkV5QCM7NN_cjs.dispose.call(this.o, false);
        this.c = [];
      ...
      for (j = 0; j < newLen; j++) {
        this.g[j] = newItems[j];
        this.f[j] = chunkV5QCM7NN_cjs.compute(
          this.c[j] = chunkV5QCM7NN_cjs.createComputation(j, null),
          mapper,
          null
        );
      }
      ...
      for (j = start; j < newLen; j++) {
        if (j in temp) {
          ...
          chunkV5QCM7NN_cjs.write.call(this.c[j], j);
        } else {
          this.f[j] = chunkV5QCM7NN_cjs.compute(
            this.c[j] = chunkV5QCM7NN_cjs.createComputation(j, null),
            mapper,
