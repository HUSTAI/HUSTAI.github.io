import { AUDIO_PROVIDER } from './chunk-UCGBCHSV.js';
import { HLS_PROVIDER, SET_AUTO_QUALITY, ENABLE_AUTO_QUALITY, coerceToError } from './chunk-Y75S5NJU.js';
import { VIDEO_PROVIDER } from './chunk-JTNFE2W4.js';
import { LIST_READONLY, LIST_ADD, LIST_REMOVE, LIST_ON_REMOVE, LIST_RESET, LIST_SET_READONLY, LIST_ON_RESET, LIST_ON_USER_SELECT, LIST_SELECT } from './chunk-DKC3YJES.js';
...
import { isTrackCaptionKind, ATTACH_VIDEO, TEXT_TRACK_NATIVE_HLS, TextTrack, TEXT_TRACK_CROSSORIGIN, TEXT_TRACK_CAN_LOAD, TEXT_TRACK_ON_MODE_CHANGE, TEXT_TRACK_NATIVE, TEXT_TRACK_UPDATE_ACTIVE_CUES } from './chunk-QUTZHJNC.js';
...
var FullscreenController = class extends ComponentController {
  ...
  onConnect() {
    listenEvent(fscreen_esm_default, "fullscreenchange", this.Mb.bind(this));
    listenEvent(fscreen_esm_default, "fullscreenerror", this.nb.bind(this));
  }
  async onDisconnect() {
    if (CAN_FULLSCREEN)
      await this.exit();
...
var ScreenOrientationController = class extends ComponentController {
  constructor() {
    super(...arguments);
    this.Ua = signal(getScreenOrientation());
   ...
   */
  get portrait() {
    return this.Ua().startsWith("portrait");
  }
  /**
   ...
   */
  get landscape() {
    return this.Ua().startsWith("landscape");
  }
  /**
  ...
  gd(event) {
    this.Ua.set(getScreenOrientation());
    this.dispatch("orientation-change", {
      detail: {
        orientation: peek(this.Ua),
        lock: this.ob
      },
      trigger: event
    });
   ...
   */
  async lock(lockType) {
    if (peek(this.Aa) || this.ob === lockType)
      return;
...
var MediaKeyboardController = class extends ComponentController {
  ...
  Ff() {
    ...
    effect(() => {
      if (!$active())
        return;
      listenEvent(target, "keyup", this.$a.bind(this));
      listenEvent(target, "keydown", this.ab.bind(this));
      listenEvent(target, "keydown", this.Gf.bind(this), { capture: true });
    });
  }
  $a(event) {
    ...
    if (method2?.startsWith("seek")) {
      ...
      if (this.Ga) {
        ...
        this.a.remote.seek(this.vb, event);
        this.vb = void 0;
      }
    }
  ...
  ab(event) {
    ...
    const sliderFocused = focused?.hasAttribute("data-media-slider"), method2 = this.cc(event);
  ...
  Gf(event) {
    if (isHTMLMediaElement(event.target) && this.cc(event)) {
      event.preventDefault();
    }
  }
  cc(event) {
  ...
  Hf(event, type) {
    ...
    return this.vb = Math.max(
      0,
      Math.min(
        (this.vb ?? this.$store.currentTime()) + (type === "seekForward" ? +seekBy : -seekBy),
        this.$store.duration()
      )
    );
  }
  zd(event) {
    this.Ga?.dispatchEvent(new DOMEvent(event.type, { trigger: event }));
  }
  ca(event, type) {
    if (!this.$store.canSeek())
      return;
    if (!this.Ga)
      this.Ga = this.el.querySelector("media-time-slider");
    if (this.Ga) {
      this.zd(event);
    } else {
      this.a.remote.seeking(this.Hf(event, type), event);
    }
  }
};
var SYMBOL_KEY_MAP = ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")"];
...
var TimeRange = class {
  ...
  start(index) {
    return this.Ha[index][0] ?? Infinity;
  }
  end(index) {
    return this.Ha[index][1] ?? Infinity;
  }
};
...
var VideoQualityList = class extends SelectList {
   ...
   */
  autoSelect(trigger) {
    if (this.readonly || this.wb || !this[ENABLE_AUTO_QUALITY])
      return;
...
var MediaUserController = class extends ComponentController {
   ...
   */
  idle(idle, delay = this.bb, trigger) {
    this.Bd();
    if (!this.ec)
  ...
  onConnect() {
    ...
    listenEvent(this.el, "play", this.Kf.bind(this));
    listenEvent(this.el, "pause", this.Lf.bind(this));
  }
  fc() {
    if (this.$store.paused())
      return;
    const onStopIdle = this.Mf.bind(this);
  ...
  Dd(idle, trigger) {
    ...
    if (idle && document.activeElement && this.el?.contains(document.activeElement)) {
      this.cb = document.activeElement;
...
var TextRenderers = class {
  constructor(_media) {
    ...
    listenEvent(textTracks, "add", this.Wb.bind(this));
    listenEvent(textTracks, "remove", this.kg.bind(this));
    listenEvent(textTracks, "mode-change", this.S.bind(this));
  }
  jg() {
  ...
  add(renderer) {
    this.yb.push(renderer);
    this.S();
  }
  remove(renderer) {
    renderer.detach();
    this.yb.splice(this.yb.indexOf(renderer), 1);
    this.S();
  }
  /* @internal */
  [ATTACH_VIDEO](video) {
    requestAnimationFrame(() => {
      this.k = video;
      if (video) {
        this.R = new NativeTextRenderer();
        this.R.attach(video);
        for (const track of this.nc)
          this.Td(track);
      }
      this.S();
    });
  ...
  Td(track) {
    if (!isTrackCaptionKind(track))
      return;
    this.R?.addTrack(track);
  }
  lg(track) {
    if (!isTrackCaptionKind(track))
      return;
    this.R?.removeTrack(track);
  }
  Wb(event) {
    this.Td(event.detail);
  }
  kg(event) {
    this.lg(event.detail);
  }
  S() {
    if (!this.k) {
      this.Sd();
    ...
    const currentTrack = this.nc.selected;
    if (this.Rd || currentTrack?.[TEXT_TRACK_NATIVE_HLS]) {
      this.na?.changeTrack(null);
      this.R.setDisplay(true);
      this.R.changeTrack(currentTrack);
      return;
    }
    this.R.setDisplay(false);
    this.R.changeTrack(null);
    if (!currentTrack) {
      this.na?.changeTrack(null);
      return;
    }
    const customRenderer = this.yb.sort((a, b) => a.priority - b.priority).find((loader) => loader.canRender(currentTrack));
...
var TextTrackList = class extends List {
  ...
  add(init, trigger) {
    ...
    if (this.zb[init.kind] && init.default)
      delete init.default;
  ...
  remove(track, trigger) {
    if (!this.q.includes(track))
      return;
    if (track === this.zb[track.kind])
      delete this.zb[track.kind];
    track.mode = "disabled";
...
var MediaPlayerDelegate = class {
  ...
  async Jf() {
    ...
    try {
      await player.play();
      this.e("autoplay", { detail: { muted: $store.muted() } });
    } catch (error) {
      this.e("autoplay-fail", {
        detail: {
          muted: $store.muted(),
          error
        }
...
var Queue = class {
  ...
  l(key2, item) {
    if (!this.g.has(key2))
      this.g.set(key2, /* @__PURE__ */ new Set());
    this.g.get(key2).add(item);
  }
  /**
   * Process all items in queue for the given `key`.
   */
  Rb(key2, callback) {
    const items = this.g.get(key2);
    if (items)
...
var MediaRequestManager = class extends ComponentController {
  constructor(instance, _stateMgr, _request, _media) {
    ...
    this.db = new MediaUserController(instance);
    this.Ja = new FullscreenController(instance);
    this.L = new ScreenOrientationController(instance);
  }
  onConnect() {
    effect(this.Nf.bind(this));
    effect(this.Of.bind(this));
    effect(this.Pf.bind(this));
    const names = Object.getOwnPropertyNames(Object.getPrototypeOf(this)), handle = this.Qf.bind(this);
    for (const name of names) {
      if (name.startsWith("media-")) {
        this.listen(name, handle);
      }
    }
    this.listen("fullscreen-change", this.Mb.bind(this));
  }
  Qf(event) {
    event.stopPropagation();
    if (peek(this.j))
  ...
  async Gd(target = "prefer-media") {
    const provider = peek(this.j);
    const adapter = target === "prefer-media" && this.Ja.supported || target === "media" ? this.Ja : provider?.fullscreen;
  ...
  async Hd(target = "prefer-media") {
    ...
    try {
      ...
      if (this.hc && peek(this.b.canPictureInPicture)) {
        await this.jc();
      }
      return result;
    } finally {
  ...
  Nf() {
    this.db.idleDelay = this.$props.userIdleDelay();
  }
  Of() {
    const { canLoad, canFullscreen: canFullscreen2 } = this.b, supported = this.Ja.supported || this.j()?.fullscreen?.supported || false;
  ...
  ["media-audio-track-change-request"](event) {
    ...
    if (track) {
      this.f.g.l("audioTrack", event);
      track.selected = true;
    }
  }
  async ["media-enter-fullscreen-request"](event) {
    try {
      this.f.g.l("fullscreen", event);
  ...
  async Mb(event) {
    ...
    try {
      ...
      if (this.L.supported && !isUndefined(lockType)) {
        await this.L.lock(lockType);
      }
    } catch (e) {
    }
  }
  ...
  ["media-resume-user-idle-request"](event) {
    this.f.g.l("userIdle", event);
    this.db.pauseIdleTracking(false, event);
  }
  ["media-pause-user-idle-request"](event) {
    this.f.g.l("userIdle", event);
    this.db.pauseIdleTracking(true, event);
  }
...
var MediaStateManager = class extends ComponentController {
  constructor(instance, _request, _media) {
    ...
    this.lc = functionDebounce(() => {
      ...
      waiting.set(true);
      playing.set(false);
      const event = this.createEvent("waiting", { trigger: this.Ka });
  ...
  C(request, event) {
    this.f.g.Rb(request, (requestEvent) => {
  ...
  Rf() {
    ...
    listenEvent(textTracks, "add", this.kc.bind(this));
    listenEvent(textTracks, "remove", this.kc.bind(this));
  ...
  Sf() {
    ...
    listenEvent(qualities, "change", this._b.bind(this));
    listenEvent(qualities, "auto-change", this.Uf.bind(this));
    listenEvent(qualities, "readonly-change", this.Vf.bind(this));
  }
  Tf() {
    ...
    listenEvent(audioTracks, "change", this.Wf.bind(this));
  }
  kc(event) {
  ...
  ["play"](event) {
    ...
    if (ended() || this.f.Ia) {
      this.f.Ia = false;
      ended.set(false);
      this.P(this.createEvent("replay", { trigger: event }));
    }
  }
  ["play-fail"](event) {
    appendTriggerEvent(event, this.n.get("play"));
  ...
  ["started"](event) {
    ...
    if (!started()) {
      ...
      started.set(true);
      this.P(this.createEvent("started", { trigger: event }));
    }
  }
  ["pause"](event) {
    if (this.f.da) {
...
var Player = class extends Component {
  constructor(instance) {
    ...
    context.$iosControls = computed(this.ng.bind(this));
    ...
    this.Q = new MediaStateManager(instance, request, context);
    this.M = new MediaRequestManager(instance, this.Q, request, context);
    context.delegate = new MediaPlayerDelegate(
      this.Q.P.bind(this.Q),
      context
    );
  ...
  onAttach(el) {
    ...
    listenEvent(el, "find-media-player", this.wg.bind(this));
  }
  onConnect(el) {
    if (IS_IPHONE)
  ...
  set paused(paused) {
    if (paused) {
      this.W.l("paused", () => this.M.Ed());
    } else
      this.W.l("paused", () => this.M.gc());
  }
  fc() {
  ...
  async enterFullscreen(target) {
    return this.M.Gd(target);
  }
  async exitFullscreen(target) {
    return this.M.Hd(target);
  }
