...
import { signal, peek, effect } from 'maverick.js';
import { defineElement } from 'maverick.js/element';
import { listenEvent, setStyle, isNumber } from 'maverick.js/std';
...
var ChaptersMenuItems = class extends MenuItems {
  ...
  onAttach(el) {
    ...
    this.setAttributes({
      "data-thumbnails": this.Ce.bind(this)
    });
  }
  ad() {
    peek(() => this.Ma());
  ...
  Ma() {
    ...
    const { currentTime } = this.a.$store, time = currentTime(), activeCueIndex = track.cues.findIndex((cue) => isCueActive(cue, time));
