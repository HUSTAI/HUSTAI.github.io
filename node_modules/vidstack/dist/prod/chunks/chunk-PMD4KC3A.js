...
import { TextTrack } from './chunk-QUTZHJNC.js';
...
var AudioProviderLoader = class {
  ...
  async load() {
    return new (await import('./provider-MTFVPRGI.js')).AudioProvider(this.Wd);
  }
  render($store) {
    return (() => {
...
var VideoProviderLoader = class {
  ...
  async load(context) {
    return new (await import('./provider-E3FDH5XV.js')).VideoProvider(this.k, context);
  }
  render($store) {
...
var _HLSProviderLoader = class extends VideoProviderLoader {
  ...
  async load(context) {
    return new (await import('./provider-EICHTRF2.js')).HLSProvider(this.k, context);
  }
};
...
var SourceSelection = class {
  ...
  yg() {
    this.a.delegate.e("sources-change", {
      detail: [...normalizeSrc(this.a.$props.src()), ...this.Ab()]
    });
  }
  zg() {
    const { $store } = this.a;
    const sources = $store.sources(), currentSource = peek($store.source), newSource = this.Yd(currentSource, sources), noMatch = sources[0]?.src && !newSource.src && !newSource.type;
    if (noMatch) {
      ...
      Promise.all(
        ...
        if (abort.signal.aborted)
          return;
        this.Yd(peek($store.source), sources2);
        tick();
      });
      return () => abort.abort();
    }
  ...
  Yd(currentSource, sources) {
    let newSource = { src: "", type: "" }, newLoader = null;
    for (const src of sources) {
      const loader = peek(this.Xd).find((loader2) => loader2.canPlay(src));
      if (loader) {
        newSource = src;
        newLoader = loader;
      }
    }
    this.Cg(currentSource, newSource, newLoader);
    this.Dg(peek(this.Na), newLoader);
    return newSource;
  }
  Cg(currentSource, newSource, newLoader) {
    if (newSource.src === currentSource.src && newSource.type === currentSource.type)
      return;
    this.a.delegate.e("source-change", { detail: newSource });
    this.a.delegate.e("media-type-change", {
      detail: newLoader?.mediaType(newSource) || "unknown"
    });
  }
  Dg(currentLoader, newLoader) {
    if (newLoader === currentLoader)
      return;
    this.a.delegate.e("provider-change", { detail: null });
    newLoader && peek(() => newLoader.preconnect?.(this.a));
    this.Na.set(newLoader);
    this.a.delegate.e("provider-loader-change", { detail: newLoader });
  }
  Ag() {
...
var Tracks = class {
  ...
  Eg() {
    const newTracks = [...this.a.$props.textTracks(), ...this.Bb()];
    for (const oldTrack of this.Zd) {
...
var Outlet = class extends Component {
  constructor(instance) {
    ...
    this.Na = signal(null);
    this.a = useMedia();
    new SourceSelection(this.Ab, this.a, this.Na);
    new Tracks(this.Bb, this.a);
  }
  onAttach(el) {
    el.setAttribute("keep-alive", "");
  }
  onConnect(el) {
    const resize = new ResizeObserver(animationFrameThrottle(this.G.bind(this)));
    resize.observe(el);
    const mutation = new MutationObserver(this.Cb.bind(this));
  ...
  render() {
    ...
    return () => {
      ...
      const loader = this.Na();
      if (!loader)
        return null;
      const el = loader.render(this.a.$store);
      peek(() => {
        loader.load(this.a).then((provider) => {
          if (peek(this.Na) !== loader)
            return;
          this.a.delegate.e("provider-change", {
            detail: provider
          });
