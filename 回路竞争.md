...
# LLM如何重映现实世界（二）：LLM中的信息回路


本文主要介绍有关LLM中的知识回路以及一些相关的猜想，有关LLM在完成任务过程中，信息进行传播的方式。
